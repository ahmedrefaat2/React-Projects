import { Trash, Pencil, Check, Activity } from "lucide-react";
...
function Tasks({newTask, onComplete, onDelete, onEdit}) {
  return (
    <div className="hover:scale-105 transform transition-transform duration-300 w-full rounded-lg bg-[#2B3A8C] p-4 text-white flex items-center place-content-between h-18">
      <div>
      <p className="text-3xl">{newTask.title}</p>
      <p className="text-sm opacity-80">{newTask.description}</p>
      </div>
      ...
      <Button type="button" onClick={onEdit} className="cursor-pointer w-8 h-8 rounded-md border-2 border-blue-500 bg-white text-blue-500 active:translate-y-[1.5px] hover:bg-blue-200"><Pencil/></Button>
